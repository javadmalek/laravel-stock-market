            ...
            <div class="col-md-8 col-md-offset-2">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <!--begin::Form-->
                        {{ Html::ul($errors->all()) }}
                        {{ Form::model($stock, array('route' => array('stocks.update', $stock->id), 'method' => 'PUT', 'class' => 'form-horizontal', 'id' => '"maskForm"')) }}
                                ...
                                <div class="m-radio-inline">
                                    <select name="_company_id" id="_company_id" class="form-control m-input">
                                        @foreach(Auth::user()->companies as $company)
                                            <option value="{{$company->id}}" @if($company->id === Input::old('_company_id')) selected="selected" @endif>{{$company->title}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="m-radio-inline">
                                    <select name="_market_id" id="_market_id" class="form-control m-input">
                                        @foreach(Auth::user()->markets as $market)
                                            <option value="{{$market->id}}" @if($market->id === Input::old('_market_id')) selected="selected" @endif>{{$market->title}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="form-group">
                            <div class="col-md-6 col-md-offset-4">
                                {{ Form::submit('Update Stock!', array('class' => 'btn btn-success')) }}
